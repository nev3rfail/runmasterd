...
use anyhow::Result;
use crate::util::elevate_self;
//extern crate native_windows_gui as nwg;

pub fn configurator() -> Result<()> {
    println!("configurator");
    // if let ControlFlow::Break(e) = elevate_self() {
    //     return e;
    // }
    // 
    println!("?!?!??!?!??AAAAAA {:?}", find_startup_entries());
    conf();
    Ok(())
}
...
use std::rc::Rc;
use crate::startup::{find_startup_entries, get_startup_folder_path};

fn conf() {
    nwg::init().expect("Failed to init Native Windows GUI");
    unsafe {
        nwg::set_dpi_awareness();
        println!("dpi: {}", nwg::scale_factor());
    }
    ...
    nwg::Button::builder()
        .text(get_startup_folder_path().unwrap().to_str().unwrap())
        .parent(&window)
